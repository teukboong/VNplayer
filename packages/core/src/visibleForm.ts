...
function narrativeLevelInstruction(level: 1 | 2 | 3): string {
  if (level === 1) {
    return [
      "전개 속도는 1, 느림입니다.",
      "상태 변화 주기는 1회입니다: 플레이어 행동의 실행 완료, 바로 닿는 반응, 작은 변화나 다음 압력만 다룹니다.",
  ...
  if (level === 3) {
    return [
      "전개 속도는 3, 빠름입니다.",
      "상태 변화 주기는 2-3회입니다: 실행, 반작용/복잡화, 새 위치나 대가가 드러나는 후과까지 반드시 이어 갑니다.",
  ...
  return [
    "전개 속도는 2, 보통입니다.",
    "상태 변화 주기는 2회입니다: 플레이어 행동의 직접 결과와 그 결과가 만든 반작용/여파까지 다룹니다.",
...
function detailLevelInstruction(level: 1 | 2 | 3): string {
  if (level === 1) {
    return [
      "묘사 밀도는 1, 간결입니다.",
      "분량 계약은 6-10문단, 공백 제외 최소 650자입니다. 극단적인 예외가 아니면 6문단 미만으로 제출하지 마세요.",
  ...
  if (level === 3) {
    return [
      "묘사 밀도는 3, 풍부입니다.",
      "분량 계약은 18-24문단, 공백 제외 최소 1800자입니다. 극단적인 예외가 아니면 18문단 미만이나 1800자 미만으로 제출하지 말고, 28문단을 넘기지 마세요.",
      "짧은 문단으로 문단 수만 채우지 말고, 문단당 공백 제외 평균 90-130자 정도의 장면 밀도를 유지하세요.",
  ...
  return [
    "묘사 밀도는 2, 표준입니다.",
    "분량 계약은 12-18문단, 공백 제외 최소 1400자입니다. 극단적인 예외가 아니면 12문단 미만이나 1400자 미만으로 제출하지 마세요.",
    "짧은 문단으로 문단 수만 채우지 말고, 문단당 공백 제외 평균 80-120자 정도의 장면 밀도를 유지하세요.",
...
export function buildVisibleTurnForm(input: BuildVisibleTurnFormInput): VisibleTurnForm {
  ...
  return {
    ...
    instruction: [
      ...
      "낯선 언어, 제한된 이해, 소통 불안이 있어도 인물의 대사를 설명문처럼 깨뜨리지 마세요. 인물은 자기 목적, 감정, 사회적 위치, 현재 위험 인식에 맞게 말하고, 주인공이 완전히 이해하지 못한 내용은 서술에서 알아들은 뜻과 놓친 부분으로 제한해 처리하세요.",
      "적대자나 주변 인물의 대사는 선택지, 지형, 규칙, 상태이상을 안내하는 UI 문구가 아니어야 합니다. 필요한 정보는 명령, 위협, 협상, 오해, 실수로 새어 나오는 말, 행동 반응 속에 묻고, 나머지는 장면 변화와 선택면으로 드러내세요.",
      ...
      "구체적 표면과 물리적 인과는 장면을 붙잡기 위한 장식이 아니라 상태를 바꾸는 원인으로 사용하세요. 새 표면을 도입할 때는 위치, 상대 상태, 자원, 탈출 가능성, 정보, 관계 중 무엇이 달라졌는지 함께 드러내세요.",
      "같은 표면을 반복해서 조작하는 장면은 압축하세요. 반복이 필요하면 각 반복이 장소 경계, 상대 상태, 도구 소유권, 부상/자원, 정보/관계 중 하나를 실제로 바꾸어야 합니다.",
      "핵심 변화 1-2곳에서는 물리적 인과와 감각의 방향을 선명하게 쓰세요. 어떤 움직임이 무엇을 누르고, 그 압력이 어떤 소리나 흔적을 만들었는지 독자가 위치를 잃지 않을 만큼만 보여 주세요.",
      "윤문에서는 물리적 인과와 독자의 방향감을 우선하세요. 인지 상태를 설명해야 할 때도 추상어에만 기대지 말고, 감각의 이동, 몸의 반응, 사물의 작동, 위협의 거리 변화와 함께 드러내세요.",
      "반복 지칭과 공간 정보는 장면을 안정시키는 데 필요한 만큼 사용하되, 리듬이 단조로워질 때는 행동 주어, 시선 이동, 소리의 방향, 압력의 변화로 변주하세요.",
      "물건과 장치의 의미는 가능한 한 사용 중의 흔적과 반응으로 전달하고, 판단 유보 표현은 장면의 긴장에 맞게 압축하세요.",
      ...
      "장면을 성급하게 건너뛰지 말되, 이미 선택된 행동의 직접 목표는 한 턴 안에서 처리하세요. 핵심 갈등 전체를 해결하지 않더라도, 현재 행동이 만든 국면은 다음 문제로 넘어가야 합니다.",
      "선택지 직전의 새 상태는 같은 교착의 미세 조정이 아니라 독자가 다른 종류의 결정을 할 수 있는 상태여야 합니다. 이동, 손실, 상대의 상태 변화, 정보 공개, 관계 태도 변화, 새 공간 진입 중 하나가 드러나야 합니다.",
      "같은 장소, 같은 핵심 상대, 같은 핵심 도구, 같은 목표가 2턴 이상 유지되면 다음 턴은 현재 교착을 끝내거나 다른 국면으로 전환하세요. 전환은 탈출, 포획, 상대의 이탈/무력화, 장소 경계 돌파, 관계/정보의 명확한 반전, 회복 불가능한 자원 손실 중 하나일 수 있습니다.",
      ...
      "선택지의 label, action, tag, intent는 모두 독자에게 보이는 UI 문구입니다. 고유명사를 제외하고 한국어로 작성하고, advance, probe, enter, time_pass 같은 영어 메타 태그를 그대로 쓰지 마세요.",
      "선택지 1-4 중 최소 1개는 현재 교착을 끝내거나 장면을 다른 국면으로 넘기는 행동이어야 합니다. 단순한 추가 조작, 관찰, 압박 유지 선택지만으로 선택지를 채우지 마세요.",
      "StoryTurn에는 concreteDelta를 반드시 포함해 주세요. 이번 턴에서 실제로 달라진 구체적 가시 변화 한 문장이어야 하며, 백엔드가 대신 만들 수 없습니다. 각도, 자세, 압력 변화만으로 끝내지 말고 장소 경계, 상대 수/상태, 도구 소유권, 탈출 가능성, 부상/자원, 관계/정보 중 하나가 되돌리기 어렵게 달라졌는지 포함하세요.",
