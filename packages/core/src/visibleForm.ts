...
function narrativeLevelInstruction(level: 1 | 2 | 3): string {
  if (level === 1) {
    return [
      "서사 레벨은 1입니다.",
      "진행 예산은 1 beat입니다: 플레이어 행동의 직접 실행, 바로 닿는 반응, 다음 압력만 다룹니다.",
      "출력량은 의도적으로 짧아야 하지만 충분한 체류감은 있어야 합니다. 대략 8-12개 문단, 레벨 2의 절반 이하 분량 안에서 한 가지 핵심 움직임과 장면 표면 하나의 변화만 선명하게 남기세요.",
      "감각 묘사는 짧게 두고, 바뀐 위치나 즉각적인 반응을 중심으로 작성하세요.",
      "새 장소로 크게 건너뛰거나 새 사건을 두 개 이상 열지 말고, 카메라를 가까이 붙여 즉각적인 결과에서 멈추세요."
    ].join(" ");
  }
  if (level === 3) {
    return [
      ...
      "출력량은 눈에 띄게 길어야 합니다. 대략 32-48개 문단도 자연스럽게 허용하되, 문단 수보다 사건 좌표 변화가 더 중요합니다.",
      "공간의 위치, 위험의 거리, 관계의 태도, 자원/상처/시간 압력, 새로 조작 가능한 표면 중 최소 세 가지가 실제로 바뀌어야 합니다.",
      "첫 1 beat는 플레이어 행동을 실제로 실행하고, 둘째 beat는 장면이 그 행동에 반작용하게 만들고, 셋째 beat는 새 위치, 새 압박, 새 표면, 새 대가 중 하나가 독자가 선택할 수 있는 상태로 착지해야 합니다.",
      "분량이 길어질수록 같은 순간을 확대하기보다, 행동의 결과가 위치, 위험, 자원, 관계, 출구 중 여러 축으로 파급되어야 합니다.",
      "선택 행동이 accepted라면 마지막까지 같은 행동을 계속 시도 중인 상태로 멈추지 마세요. blocked나 partial이 아니라면, 그 행동은 한 턴 안에서 충분히 성립한 뒤 다음 문제로 넘어가야 합니다.",
      "다음 선택지는 방금 선택한 행동의 단순 반복이 아니라, 그 행동이 만든 새 상태에서 가능한 후속 행동이어야 합니다.",
      "핵심 사건을 완전히 해결하지는 말되, 레벨 1에서 멈췄을 지점을 지나 다음 국면의 문턱까지 장면을 전진시키세요."
    ].join(" ");
  }
  return [
    "서사 레벨은 2입니다.",
    "진행 예산은 2 beat입니다: 플레이어 행동의 직접 결과와 그 결과가 만든 반작용/여파까지 다룹니다.",
    "출력량은 중간 길이여야 합니다. 대략 16-24개 문단, 레벨 1보다 명확히 길고 레벨 3보다 명확히 짧게 쓰며 감각, 외부 압력, 선택 가능한 표면이 최소 한 번씩 변화해야 합니다.",
    "레벨 1보다 한 박자 더 멀리 가되, 레벨 3처럼 새 국면 전체를 열기 전 다음 선택 지점에서 멈추세요."
  ].join(" ");
}
...
export function buildVisibleTurnForm(input: BuildVisibleTurnFormInput): VisibleTurnForm {
  return {
    ...
    instruction: [
      ...
      "분량은 padding이 아니라 진행 예산입니다. 레벨 1은 가까운 직접 반응, 레벨 2는 결과와 여파, 레벨 3은 실행-복잡화-후과의 연쇄 진행으로 체감 차이가 나야 합니다.",
      "출력량 차이도 반드시 보여야 합니다. 레벨 1은 8-12문단의 압축 체류, 레벨 2는 16-24문단의 중간 체류, 레벨 3은 32-48문단도 허용되는 긴 체류감으로 작성하세요.",
      "서사 레벨이 높아질수록 같은 순간을 더 오래 늘이는 것이 아니라, 행동의 결과가 더 멀리 파급되어야 합니다. 장면 안에서 달라진 위치, 관계, 위험, 자원, 조작 가능한 표면의 개수가 늘어나야 합니다.",
