/**
 * VNplayer Worker: stable front door for a rotating local MCP tunnel origin.
 *
 * KV key: "origin" (default)
 * Secret: ORIGIN_UPDATE_SECRET (required for /_vnplayer/origin)
 */

const DEFAULT_ORIGIN_KV_KEY = "origin";
const DEFAULT_ALLOWED_ORIGIN_SUFFIXES = "trycloudflare.com,ts.net,loca.lt";
const ADMIN_ORIGIN_PATH = "/_vnplayer/origin";
const HEALTH_PATH = "/_vnplayer/healthz";

const PROXY_PATH_PREFIXES = [
  "/mcp",
  "/api/health",
  "/api/webgpt/manifest",
  "/api/webgpt/openapi.json",
  "/api/webgpt/tools/"
];
...
async function updateOrigin(request, env, key) {
  ...
  const got = request.headers.get("X-VNplayer-Origin-Update-Secret") || "";
  ...
  await env.VNPLAYER_KV.put(key, normalized.origin);
...
async function proxyToOrigin(request, env, key) {
  const origin = (await env.VNPLAYER_KV.get(key)) || "";
  if (!origin) {
  ...
  headers.set("X-VNplayer-External-Host", incoming.host);
  headers.set("X-VNplayer-External-Proto", incoming.protocol.replace(":", ""));
